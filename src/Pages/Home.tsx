...
export default function Home() {
  ...
  const [todoListToShow, setTodoListToShow] = useState<string | number | null>(
    null,
  );
  ...
  function selectList(item: TodoListType) {
    setTodoListToShow(item.id || null);
    setShowModal(true);
  }
    ...
    <div className="box-border grid h-screen w-screen grid-rows-[90px_1fr] overflow-auto bg-black text-slate-300 dark:[color-scheme:dark]">
      ...
      <main className="container relative m-auto px-4 py-10 sm:px-16">
        <section className="z-20 flex flex-wrap items-center justify-center gap-8">
          {todoLists?.map((list: TodoListType, idx: number) => (
            <TodoList
              key={list.id}
              todoList={list}
      ...
      {showModal && (
        ...
        >
          <ModalViewList
            todoList={
              todoLists?.find((report) => report.id == todoListToShow) ?? null
            }
          />
        </div>
      )}
